...
-   **Process**:
    ...
    -   Upon successful download, `electron-updater` notifies the main process. The main process then sends an IPC message (`update:downloaded`) to the renderer process (`App.tsx`). The renderer listens for this event and displays a non-intrusive notification component (`UpdateNotification.tsx`) to the user. If the user chooses to install, the renderer sends another IPC message (`updater:quit-and-install`) back to the main process, which then calls `autoUpdater.quitAndInstall()`.
